...
class LabelUtils:
    ...
    @staticmethod
    def json_to_YOLO_lbls(img_pth_lst, jsn_pth_lst, save_path):
        # generates img_name.txt label file for every image with a coco json label  
        assert len(img_pth_lst) == len(jsn_pth_lst)
        lbl_len = len(jsn_pth_lst)
        for i in range(lbl_len):
            jsonfile = jsn_pth_lst[i]
            jf = open(jsonfile)
            data = json.load(jf)
            try:
                img_name = data['image']['filename']
                img_path = img_pth_lst[i]
                imh, imw = Utils.get_shape_pil(img_path)
                label_list = []
    ...
    @staticmethod
    def lbl_masks_to_img_lbls(img_pth_lst, lbl_msk_pths, save_path, color=None):
        ...
        assert len(img_pth_lst) == len(lbl_msk_pths)
        lbl_len = len(lbl_msk_pths)
        for i in range(lbl_len):
            msk_path = lbl_msk_pths[i]
            img_path = img_pth_lst[i]
    ...
    @staticmethod
    def lbl_masks_to_contours(img_pth_lst, lbl_msk_pths, save_path, color=None):
        assert len(img_pth_lst) == len(lbl_msk_pths)
        lbl_len = len(lbl_msk_pths)
        for i in range(lbl_len):
            msk_path = lbl_msk_pths[i]
    ...
    @staticmethod
    def convert_predict_df_to_yolo_labels(df: pd.DataFrame, confidence_thresh: float, batch_output_dir: str):
        """
        Convert a dataframe with object prediction information to image-wise YOLO format labels.
        
        Args:
            df (pd.DataFrame): Dataframe containing the predicted box coordinates.
            batch_output_dir (str): The specific batch directory to save the labels.
        """
        
        # 1. Define and create the label output directory
        output_dir = pathlib.Path(batch_output_dir) / "yolo_labels"
        os.makedirs(output_dir, exist_ok=True)

        # 2. Check if the necessary predicted columns are present
        # Using the '_p' columns as they represent the prediction/YOLO output data.
        required_columns = ['Filename', 'cls', 'x', 'y', 'w', 'h', "conf"]
        if not all(col in df.columns for col in required_columns):
            raise ValueError(f"DataFrame must contain the following columns: {', '.join(required_columns)}")

        # 3. Inner function to process each image group and save the YOLO label file
        def save_yolo_labels(group, threshold):
            """
            Formats the grouped DataFrame into YOLO label strings and saves them to a .txt file.
            YOLO format: <class_id> <x_center> <y_center> <width> <height> (normalized 0-1)
            """
            # Select the class ID and the normalized box coordinates for PREDICTION
            yolo_data = group[group['conf'] >= threshold]
            yolo_coords = yolo_data[['cls', 'x', 'y', 'w', 'h']].values
            
            lines = []
            for row in yolo_coords:
                # Assuming 'cls' is the first column in the slice
                if pd.isna(row[0]):
                    continue  # Skip rows with no class ID
                
                # Format: Class ID as integer, coordinates as floats with precision
            ...
            label_path = output_dir / f"{base_filename}.txt"
            
            # Write the labels to the file
            if lines: # Only write file if there are labels
                with open(label_path, 'w') as f:
                    f.write('\n'.join(lines))

        # 4. Group by Filename and apply the function to create label files
        # The use of .apply() on groupby is Pythonic for this task.
        labels_per_file = df.groupby('Filename').apply(save_yolo_labels, threshold=confidence_thresh)
        
        # 5. Save a labels.txt file with the full file paths (Optional but retained)
        label_files = [str(output_dir / label_file) for label_file in os.listdir(output_dir) if label_file.endswith(".txt")]
        labels_txt_path = output_dir.parent / "labels.txt" # saves it one level up from yolo_labels
        
        with open(labels_txt_path, 'w') as f:
            f.write('\n'.join(label_files) + '\n')

        # Calculate total labels written for debugging
        total_labels_written = labels_per_file.sum()

        print(f"   Generated {len(label_files)} YOLO label files in the '{output_dir}' directory.")
        print(f"   Total bounding boxes written (conf >= {confidence_thresh}): {total_labels_written}")
        
        return len(label_files) # Return the count of files for confirmation